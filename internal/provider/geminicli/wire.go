...
// Request is the exact Code Assist wrapper around a Gemini Content request.
// The nested request remains raw so provider fields are not silently discarded.
type Request struct {
...
func Wrap(model, project, userPromptID string, request []byte) ([]byte, error) {
	if model == "" || len(bytes.TrimSpace(request)) == 0 || !json.Valid(request) {
		return nil, errors.New("Code Assist requires model and valid request")
	}
	r := Request{Model: model, Project: project, UserPromptID: userPromptID, Request: append(json.RawMessage(nil), request...)}
...
func Unwrap(body []byte) (Result, error) {
	...
	if len(r.Response) == 0 || bytes.Equal(bytes.TrimSpace(r.Response), []byte("null")) {
		return Result{}, errors.New("Code Assist response has no response envelope")
	}
	return r, nil
}
func (c *Connector) AdaptRequest(ctx context.Context, target core.Target, binding core.Binding, body []byte) ([]byte, error) {
	...
	if binding.Codec.Variant != "code-assist" {
		return body, nil
	}
...
func (c *Connector) AdaptResponse(ctx context.Context, binding core.Binding, resp *http.Response) error {
	...
	if resp == nil || resp.Body == nil {
		return errors.New("Code Assist response is missing")
	}
	...
	_ = resp.Body.Close()
	if len(raw) > 1<<20 {
		return errors.New("Code Assist response exceeds adapter limit")
	}
