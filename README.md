...
- **Opt-in telemetry.** OTLP HTTP/protobuf and gRPC exporters support per-signal fan-out, sampling, and existing-Secret header files while the Prometheus `/metrics` route remains available. See [telemetry deployment guidance](docs/deployment.md#telemetry).
